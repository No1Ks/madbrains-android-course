...
class RepositoriesLoader {
    ...
    fun parseJson(responseText: String) {
        ...
        for (index in 0 until jsonArray.length()) {
            ...
            // owner {
            repository.owner_login               = jsonObject.getJSONObject("owner").getString("login")
            repository.owner_id                  = jsonObject.getJSONObject("owner").getInt("id")
            repository.owner_node_id             = jsonObject.getJSONObject("owner").getString("node_id")
            repository.owner_avatar_url          = jsonObject.getJSONObject("owner").getString("avatar_url")
            repository.owner_gravatar_id         = jsonObject.getJSONObject("owner").getString("gravatar_id")
            repository.owner_url                 = jsonObject.getJSONObject("owner").getString("url")
            repository.owner_html_url            = jsonObject.getJSONObject("owner").getString("html_url")
            repository.owner_followers_url       = jsonObject.getJSONObject("owner").getString("followers_url")
            repository.owner_following_url       = jsonObject.getJSONObject("owner").getString("following_url")
            repository.owner_gists_url           = jsonObject.getJSONObject("owner").getString("gists_url")
            repository.owner_starred_url         = jsonObject.getJSONObject("owner").getString("starred_url")
            repository.owner_subscriptions_url   = jsonObject.getJSONObject("owner").getString("subscriptions_url")
            repository.owner_organizations_url   = jsonObject.getJSONObject("owner").getString("organizations_url")
            repository.owner_repos_url           = jsonObject.getJSONObject("owner").getString("repos_url")
            repository.owner_events_url          = jsonObject.getJSONObject("owner").getString("events_url")
            repository.owner_received_events_url = jsonObject.getJSONObject("owner").getString("received_events_url")
            repository.owner_type                = jsonObject.getJSONObject("owner").getString("type")
            repository.owner_site_admin          = jsonObject.getJSONObject("owner").getBoolean("site_admin")
            // }
